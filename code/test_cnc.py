...
# ---------------------------------------------------------------------------
# DEPTH-ESCALATING MONSTER RE-SPLIT (split_tag). march_cu is deterministic, so
# re-racing a monster parent at the SAME depth reproduces the IDENTICAL
# children forever -- a monster that fails its first race fails every race,
# never converging. The fix escalates the parent re-split DEPTH with the
# monster's attempt count, and tags each depth's evidence with a split_tag so
# merge_jsonl_verdicts groups a parent's evidence by (parent_cube, split_tag)
# and treats each depth as an INDEPENDENT complete cover. Two guards below:
#   (i)  BACKWARD COMPAT / SAFETY GATE: real committed (untagged) evidence
#        must merge byte-for-byte as before.
#   (ii) MULTI-DEPTH CORRECTNESS: two depths of one parent are two independent
#        covers, never unioned -- closing the FALSE-UNSAT trap.
...
def test_backward_compat_real_evidence_unchanged():
    # SAFETY GATE. Merge over the REAL committed shard-*.jsonl evidence for
    # every recorded cell -- including the t=28 cells (N=744, N=729) and the
    # parent-cube evidence present (t=26 N=644, t=27 N=665, t=28 N=744) --
    # must be IDENTICAL to the pre-change baseline captured in
    # code/test_cnc_merge_baseline.json (generated by running merge on this
    # exact evidence BEFORE the split_tag change). All committed evidence is
    # untagged, so the new (parent_cube, split_tag) grouping must collapse to
    # the single default group and change NOTHING: verdict, ncubes,
    ...
    for key, exp in sorted(baseline.items()):
        t, N = exp["t"], exp["N"]
        result, paths = lib.filtered_verdict(t, N, dirs)
        residual = sorted(result.get("cubes_without_unsat") or [])
        sha = hashlib.sha256(json.dumps(residual).encode()).hexdigest()
        parents_resolved = {str(p): bool(info.get("resolved"))
                            for p, info in (result.get("parents") or {}).items()}
        assert result["verdict"] == exp["verdict"], \
            (key, "verdict", result["verdict"], exp["verdict"])
        assert result["ncubes"] == exp["ncubes"], \
            (key, "ncubes", result["ncubes"], exp["ncubes"])
        assert result.get("n_cubes_refuted") == exp["n_cubes_refuted"], \
            (key, "n_cubes_refuted", result.get("n_cubes_refuted"),
             exp["n_cubes_refuted"])
        assert sorted(result.get("sat_cubes") or []) == sorted(exp["sat_cubes"]), \
            (key, "sat_cubes", result.get("sat_cubes"), exp["sat_cubes"])
        assert len(residual) == exp["cubes_without_unsat_len"], \
            (key, "residual len", len(residual), exp["cubes_without_unsat_len"])
        assert sha == exp["cubes_without_unsat_sha256"], \
            (key, "residual CHANGED vs baseline")
        assert parents_resolved == exp["parents_resolved"], \
            (key, "parent resolution CHANGED vs baseline")
        checked += 1
    assert checked >= 2, ("expected >=2 real cells (incl. the t=28 cells) in "
...
def _write_tagged_child_jsonl(d, parent, n_children, verdicts, split_tag,
                              subdir):
    """A DEPTH-TAGGED distributed re-split child-conquer JSONL: like
    _write_child_jsonl but with a `split_tag` in the meta line (the
    march_opts the parent was split with). Each write goes in its OWN
    subdirectory so two depth-splits of the same parent never collide on the
    shard-*.jsonl basename (merge globs recursively)."""
    dd = os.path.join(d, subdir)
    os.makedirs(dd, exist_ok=True)
    members = list(range(n_children))
    path = os.path.join(dd, "shard-0.jsonl")
    with open(path, "w") as f:
        f.write(json.dumps({"meta": True, "shard": 0, "nshards": 1,
                            "ncubes": n_children, "parent_cube": parent,
                            "split_tag": split_tag, "mode": "round-robin",
                            "members": members}) + "\n")
        for g in members:
            if g in verdicts:
                f.write(json.dumps({"gidx": g, "verdict": verdicts[g],
...
def _multidepth_fixture(d, d12_verdicts, d16_verdicts):
    """8-cube top-level campaign with cube 3 (the "parent") left unresolved at
    top level, plus a distributed re-split of cube 3 at TWO depths: tag
    "-d 12" (3 children) and tag "-d 16" (5 children). Returns the merge."""
    base = os.path.join(d, "base")
    os.makedirs(base)
    _write_jsonl(base, 0, 8, 2, {0: "UNSAT", 2: "UNSAT", 4: "UNSAT", 6: "UNSAT"})
    _write_jsonl(base, 1, 8, 2, {1: "UNSAT", 5: "UNSAT", 7: "UNSAT"})
    # cube 3 deliberately left unresolved at the top level.
    _write_tagged_child_jsonl(d, parent=3, n_children=3, verdicts=d12_verdicts,
                              split_tag="-d 12", subdir="p3_d12")
    _write_tagged_child_jsonl(d, parent=3, n_children=5, verdicts=d16_verdicts,
                              split_tag="-d 16", subdir="p3_d16")
    return merge_jsonl_verdicts(d)


def test_multidepth_no_crash_on_differing_child_counts_across_tags():
    # (a) Two DIFFERENT tags for the same parent, with DIFFERENT child counts
    # (3 vs 5), must NOT raise -- pre-change this crashed with a
    # "disagree on child count" ValueError. Different tags are different
    # splits; only a disagreement WITHIN one tag is corruption.
    with tempfile.TemporaryDirectory() as d:
        m = _multidepth_fixture(d, {0: "UNSAT", 1: "UNSAT"},
                                {0: "UNSAT", 1: "UNSAT"})
    # merged without exception; parent detail carries BOTH tag groups.
    assert set(m["parents"][3]["tags"]) == {"-d 12", "-d 16"}, m["parents"][3]


def test_multidepth_any_complete_cover_resolves_unsat():
    # (b) "-d 16" covers all 5 children UNSAT (a COMPLETE cover) while "-d 12"
    # got only 2/3 -- any ONE complete cover suffices, so parent 3 is resolved
    # UNSAT and the whole instance reads UNSAT (8/8 refuted).
    with tempfile.TemporaryDirectory() as d:
        m = _multidepth_fixture(
            d, {0: "UNSAT", 1: "UNSAT"},                       # d12: 2/3
            {0: "UNSAT", 1: "UNSAT", 2: "UNSAT", 3: "UNSAT", 4: "UNSAT"})  # d16: 5/5
    assert m["verdict"] == "UNSAT", m
    assert m["cubes_without_unsat"] == [], m
    assert m["parents"][3]["resolved"] is True, m["parents"][3]
    assert m["parents"][3]["closed_via_children"] is True, m["parents"][3]
    assert m["parents"][3]["tags"]["-d 16"]["closed_via_children"] is True, m
    assert m["parents"][3]["tags"]["-d 12"]["closed_via_children"] is False, m


def test_multidepth_neither_complete_does_not_resolve():
    # (c) Neither group is a complete cover (d12 2/3, d16 4/5) -- the parent
    # is NOT resolved, so the instance stays UNDETERMINED with cube 3 missing.
    with tempfile.TemporaryDirectory() as d:
        m = _multidepth_fixture(
            d, {0: "UNSAT", 1: "UNSAT"},                        # d12: 2/3
            {0: "UNSAT", 1: "UNSAT", 2: "UNSAT", 3: "UNSAT"})   # d16: 4/5
    assert m["verdict"] == "UNDETERMINED", m
    assert m["cubes_without_unsat"] == [3], m
    assert m["parents"][3]["resolved"] is False, m["parents"][3]
    assert m["parents"][3]["closed_via_children"] is False, m["parents"][3]


def test_multidepth_sat_child_in_either_group_makes_parent_sat():
    # (d) A SAT child in EITHER group decides the parent SAT (and the whole
    # instance SAT). Here the SAT child sits in the "-d 12" group.
    with tempfile.TemporaryDirectory() as d:
        m = _multidepth_fixture(
            d, {0: "UNSAT", 1: "SAT"},                          # d12 has a SAT
            {0: "UNSAT", 1: "UNSAT", 2: "UNSAT", 3: "UNSAT", 4: "UNSAT"})
    assert m["verdict"] == "SAT", m
    assert m["sat_cubes"] == [3], m
    assert m["parents"][3]["resolved"] is True, m["parents"][3]


def test_multidepth_false_unsat_trap_is_closed():
    # (e) THE CATASTROPHIC CASE. "-d 12" children {0,1} UNSAT (of 3) and
    # "-d 16" children {0,1} UNSAT (of 5) -- NEITHER group is a complete
    # cover. The pre-change merge, keying on parent index ALONE, would union
    # the two groups' local-index UNSAT sets ({0,1} u {0,1} = {0,1}) and, if
    # the counts had matched, could declare the parent UNSAT though NEITHER
    # split individually covered it -- a FALSE UNSAT. The tag grouping keeps
    # them separate, so parent 3 is NOT resolved and the instance is NOT UNSAT.
    with tempfile.TemporaryDirectory() as d:
        m = _multidepth_fixture(d, {0: "UNSAT", 1: "UNSAT"},
                                {0: "UNSAT", 1: "UNSAT"})
    assert m["verdict"] != "UNSAT", ("FALSE UNSAT: two incomplete covers "
                                     "were unioned across depths", m)
    assert m["verdict"] == "UNDETERMINED", m
    assert m["cubes_without_unsat"] == [3], m
    assert m["parents"][3]["resolved"] is False, m["parents"][3]


def test_multidepth_same_tag_count_disagreement_still_raises():
    # The within-tag corruption guard is NOT weakened: two files for the SAME
    # parent AND SAME split_tag that disagree on child count is still a
    # ValueError (genuine corruption), exactly as before.
    with tempfile.TemporaryDirectory() as d:
        _write_tagged_child_jsonl(d, parent=3, n_children=2,
                                  verdicts={0: "UNSAT"}, split_tag="-d 12",
                                  subdir="a")
        _write_tagged_child_jsonl(d, parent=3, n_children=4,
                                  verdicts={1: "UNSAT"}, split_tag="-d 12",
                                  subdir="b")
        try:
            merge_jsonl_verdicts(d)
            assert False, "expected ValueError on same-tag child-count disagreement"
        except ValueError:
            pass


def test_multidepth_same_tag_two_runs_compose():
    # Two runs at the SAME depth (same tag) compose exactly like top-level
    # re-dispatch runs: their per-child UNSATs union WITHIN the tag, and a
    # single depth's full coverage across the two runs closes the parent.
    with tempfile.TemporaryDirectory() as d:
        base = os.path.join(d, "base")
        os.makedirs(base)
        _write_jsonl(base, 0, 8, 1, {g: "UNSAT" for g in range(8) if g != 3})
        _write_tagged_child_jsonl(d, parent=3, n_children=3,
                                  verdicts={0: "UNSAT", 1: "UNSAT"},
                                  split_tag="-d 12", subdir="run1")
        _write_tagged_child_jsonl(d, parent=3, n_children=3,
                                  verdicts={2: "UNSAT"},  # the last child
                                  split_tag="-d 12", subdir="run2")
        m = merge_jsonl_verdicts(d)
    assert m["verdict"] == "UNSAT", m
    assert m["parents"][3]["tags"]["-d 12"]["closed_via_children"] is True, m


def test_untagged_and_empty_tag_are_the_same_default_group():
    # Backward-compat unit: a file with NO split_tag field and a file with
    # split_tag "" both fall in the single default group (tag None) -- so
    # legacy untagged evidence and a defaulted empty tag compose as ONE cover
    # (never split into two half-covered groups that would each fail to
    # close). Two children of parent 3: child 0 via an untagged file, child 1
    # via an empty-tag file, n_children=2 -> parent closes.
    with tempfile.TemporaryDirectory() as d:
        childdir = os.path.join(d, "untagged")
        os.makedirs(childdir)
        _write_child_jsonl(childdir, 0, parent=3, n_children=2,
                           verdicts={0: "UNSAT"})  # no split_tag field at all
        _write_tagged_child_jsonl(d, parent=3, n_children=2,
                                  verdicts={1: "UNSAT"}, split_tag="",
                                  subdir="emptytag")
        m = merge_jsonl_verdicts(d)
    assert m["parents"][3]["closed_via_children"] is True, m["parents"][3]
    assert set(m["parents"][3]["tags"]) == {""}, m["parents"][3]


def test_monster_march_opts_ladder_escalates_and_clamps():
    # The depth ladder maps a monster's prior attempt count -> the march_opts
    # for its NEXT re-split, escalating with attempts and clamping at the
    # deepest entry so a stubborn monster keeps re-racing at max depth (never
    # IndexError, never silently repeating a shallower depth -- which is the
    # whole non-convergence bug this feature fixes).
    import cnc_grind_lib as lib
    cfg = dict(lib.CONFIG_DEFAULTS)
    ladder = cfg["monster_depth_ladder"]
    assert ladder[0] == "-d 12", ladder  # attempt 0 == the original behavior
    for attempts, expected in enumerate(ladder):
        assert lib.monster_march_opts(attempts, cfg) == expected, (attempts,
                                                                   expected)
    # clamp at the deepest for any attempts beyond the ladder length.
    for attempts in (len(ladder), len(ladder) + 5, 999):
        assert lib.monster_march_opts(attempts, cfg) == ladder[-1], attempts
    # distinct depths across the first four attempts -> distinct split_tags ->
    # merge keeps them as distinct covers (never re-racing identical children).
    tags = [lib.monster_march_opts(a, cfg) for a in range(len(ladder))]
    assert len(set(tags)) == len(ladder), ("ladder depths must be distinct so "
                                           "each race produces new children", tags)


def test_monster_march_opts_empty_ladder_falls_back():
    # Degenerate config (empty ladder) must not crash -- fall back to the
    # top-level split opts so a monster still gets SOME split.
    import cnc_grind_lib as lib
    cfg = dict(lib.CONFIG_DEFAULTS)
    cfg["monster_depth_ladder"] = []
    assert lib.monster_march_opts(0, cfg) == cfg["top_march_opts"]
...
def test_plan_monster_matrix_carries_escalating_march_opts_and_tag():
    ...
    # from the ladder by each monster's recorded attempt count (0 if new). A
    # brand-new monster and one already raced twice must get DIFFERENT depths.
    import cnc_grind_lib as lib
    cfg = dict(lib.CONFIG_DEFAULTS)
    ladder = cfg["monster_depth_ladder"]
    monster_state = {"100": {"attempts": 0}, "200": {"attempts": 2}}

    def monster_opts(m):
        attempts = monster_state.get(str(m), {}).get("attempts", 0)
        return lib.monster_march_opts(attempts, cfg)

    split_matrix = [{"parent_cube": m, "march_opts": monster_opts(m),
                     "split_tag": monster_opts(m)} for m in (100, 200, 300)]
    for e in split_matrix:
        assert e["march_opts"] == e["split_tag"], e  # tag == the depth used
    by_parent = {e["parent_cube"]: e for e in split_matrix}
    assert by_parent[100]["march_opts"] == ladder[0], by_parent[100]  # new
    assert by_parent[200]["march_opts"] == ladder[2], by_parent[200]  # 2 attempts
    assert by_parent[300]["march_opts"] == ladder[0], by_parent[300]  # new
    # new vs twice-raced monster get DIFFERENT depths -> different children.
